...
`HotwireProvider` takes what belongs to a session rather than a screen, all of it optional.
The first thing most apps add is the path configuration, which lets the server decide
which URL opens as a modal or a native screen:
...
`hotwireScreens` declares a `HotwireScreen` per presentation. To change one, give it
options by presentation, the path configuration's `context` and `modal_style` words:
...
from the path configuration, and the page title as the screen title.
...
by a config, see "Your own hierarchy".
...
setting beyond these is a change to the library, not a prop.
...
the provider's token and the bridge component list, is fixed at creation.
...
`HotwireProvider` still wraps the container, and `useVisitHandler` is available to route
